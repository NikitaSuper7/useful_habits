...
def necessary_habits():
    ...
    necessary_tasks = [
        ...
        for task in tasks
        if (task.last_remind.date() + timedelta(days=task.period)) < today
        and task.owner.tg_chat_id is not None
    ]
...
def send_tg_notification():
    ...
    for task in necessary_tasks:
        ...
        if (
            datetime.strptime(task.get("time_to_do"), "%I:%M:%S") - timedelta(minutes=5)
        ).time() <= current_time and task.get("owner_chat_id"):
